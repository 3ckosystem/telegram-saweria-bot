// Telegram Mini App client — Option A (user_id opsional, boleh dibuka dari browser biasa)
// Pesan pembayaran = "INV:<invoice_id>"
// Tombol "Saya sudah bayar" melakukan polling ke /api/invoice/{id}/status
...
async function handleCheckout() {
  ...
  if (!userId) {
    qrContainer.innerHTML = `<div style="font-size:12px;color:#b25;margin:8px 0">
      Tips: sebaiknya buka via tombol Mini App di Telegram agar user terdeteksi otomatis.
    </div>`;
  }
  ...
  // 3) Tampilkan QR PNG + detail + tombol "Saya sudah bayar"
  ...
  qrContainer.innerHTML += `
    ...
    <div id="paid-info" style="font-size:12px;color:#444;margin-top:8px"></div>
  `;

  // 4) Polling status ketika user klik "Saya sudah bayar"
  document.getElementById('btn-paid')?.addEventListener('click', async () => {
    const invId = inv.invoice_id;
    const info = document.getElementById('paid-info');
    const started = Date.now();
    const timeoutMs = 120000; // 2 menit
    const sleep = (ms) => new Promise(r => setTimeout(r, ms));

    info.textContent = "Memeriksa status pembayaran...";
    while (Date.now() - started < timeoutMs) {
      try {
        const r = await fetch(`/api/invoice/${encodeURIComponent(invId)}/status`);
        if (r.ok) {
          const st = await r.json();
          if (String(st.status || "").toUpperCase() === "PAID") {
            info.textContent = "Pembayaran terdeteksi. Mengirim undangan...";
            // Sukses: tutup Mini App (kalau dibuka dari Telegram)
            try { window.Telegram?.WebApp?.close(); } catch {}
            return;
          }
        } else {
          // server belum siap / 404 -> lanjut polling
        }
      } catch (e) {
        console.log("status check error:", e);
      }
      await sleep(3000); // cek setiap 3 detik
    }
    info.textContent = "Belum terdeteksi. Jika sudah bayar, tunggu beberapa saat lalu coba lagi.";
  });
}
